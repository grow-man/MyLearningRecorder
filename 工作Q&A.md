...
## 问: VSCode 插件 `Log File Highlighter` 如何配置
...
## 问: VSCode 插件 `Doxygen Documentation Generator` 如何配置
...
## 问: CentOS7 bash 出现错误 -bash: 警告:setlocale: LC_CTYPE: 无法改变区域选项 (UTF-8): 没有那个文件或目录
...
## 问: 如何进入 docker 中某个容器的日志

```shell
docker logs <container_id_or_name>
```

## 问: 如何进入 docker 中某个容器的命令行

```shell
docker exec -it <container_id_or_name> bash
```

## 问: 如何查看 docker 中当前的正在运行的容器

```shell
docker ps
```

## 问: 一条命令导出当前分支所有 commit

```c++
git log --no-merges 721bfa7..HEAD --date=iso --pretty=format:'%h|%s|%an|%ad|' --shortstat | sed '/^$/d' | sed -n '1h;1!H;${g;s/|\n/,/g;p;}' > /home/commits.csv
```

## 问: git submodule 如何初始化特定子模块

```c++
git submodule update --init <submodule_path>
```

## 问: 为何推荐使用 `nullptr` 替代 `NULL` 和 `0`
...
## 问: 如何使用 python 遍历文件夹并生成 md 格式文件
...
## 问: 如何计算某个文件夹下，代码文件的注释占所有行数的比例

```sh
cloc --by-percent c --quiet ./ds/thirdparty
```

## 问: 如何计算某个文件夹下，代码文件的空行、注释和代码行数
...
## 问: cmake 脚本中如何让一个变量表示某文件夹下所有源文件
...
## 问：Centos 使用 yum 后使用 cmake 出现 libstdc++.so 链接错误
...
## 问：主板中 sa 软件崩溃如何使用 coredump 进行调试
...
## 问： wireshark 抓包时出现提示 failed to set hardware filter to promiscuous mode: 连到系统上的设备没有发挥作用。  (31)
...
## 问: `error while loading shared libraries` 加载动态库失败如何解决？
...
## 问： [Centos7 如何源码编译 git](https://git-scm.com/book/zh/v2/%E8%B5%B7%E6%AD%A5-%E5%AE%89%E8%A3%85-Git)
...
## 问: MySQL 使用长连接后有时因为返回的结果集过大，导致内存增长过快，如何解决?
...
## 问: 正则表达式，如何选中不带有文本 172.2.200.66 的行
...
## 问: wireshark 报文中 ICMP 报文出现 Destination unreachable (Port unreachable),为什么会出现这个错误
...
## 问: g++ 编译选项 `-Wl,--no-as-needed` 是什么意思？
...
## 问: 出现编译警告 `warning: Using 'getaddrinfo' in statically linked applications requires at runtime the shared libraries from the glibc version used for linking`
...
## 问: 如何查看 centos7 通过 yum 安装了哪些软件

```shell
yum list installed
```

## 问: 如何自动统计最近一次 git 提交中增加或删除的代码行数
...
## 问: 重启电脑后 docker desktop 一直卡在启动中
...
## 问:为何官网下载的googleTest框架的运行时库缺省为/MT 不能修改
...
## 问：调度台和调度服务器网络正常时，调度台提示连接二开调度服务器失败
...
## 问：调度台软件左下角指示灯，LTE爆红、CAD爆红、ATS爆红、NTP爆红
...
## 问：调度台视频上拉时，选择不需要确认直接拉取时手持台还需要进行确认
...
## 问：系统状态中CAD服务器、录音录像服务器图标一直成灰色，设备上线也是灰色状态，或者设备列表中调度台设备的名称为空白
...
## 问：如何进行告警配置，类似固定台的PA连接失败告警，因为固定台不需要和PA进行对接，该项告警没必要出现。
