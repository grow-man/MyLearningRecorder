...
static __int64  g_i64Tick[16][3];

#if QLZ_COMPRESSION_LEVEL == 1
#define GET_INT(s)          (*((int*)s))
#define GET_SHORT(s)        (*((short*)s))
#define SAME_INT(s, d)      (GET_INT(s) == GET_INT(d))
#define SAME_SHORT(s, d)    (GET_SHORT(s) == GET_SHORT(d))

#define same6(src)          (GET_INT(src-3) == GET_INT(src-1)) && (GET_INT(src-3) == GET_INT(src-2))
...
//#define hash_func(i)    (i & (QLZ_HASH_VALUES - 1))
//#define hash_func(i)    (((i >> 12) ^ i) & (QLZ_HASH_VALUES - 1))
#define fast_read(src, bytes)  (((bytes) > 0 && (bytes) <= 4)?*((ui32*)(src)):0)
...
#define fast_write_4(f, dst, bytes)     *((ui32*)dst) = f
#define fast_write_3(f, dst, bytes)     *((ui32*)dst) = f
#define fast_write_2(f, dst, bytes)     *((ui16*)dst) = (ui16)f
#define fast_write_1(f, dst, bytes)     *((unsigned char*)dst) = (unsigned char)f
...
#define TEST_PERF(i, j) \
{gos_time_get_tick(&g_i64Tick[i][0]); g_i64Tick[i][1] += g_i64Tick[i][0] - g_i64Tick[j][0]; g_i64Tick[i][2] ++;}
...
static size_t qlz_compress_core(const unsigned char *source, unsigned char *destination, size_t size, qlz_state_compress *state)
{
    ...
    gos_time_init();
    memset(g_i64Tick, 0, sizeof(g_i64Tick));

    //g_i64Tick[0] = gos_get_uptime_1ns();

    while(src <= last_matchstart)
    {
        gos_time_get_tick(&g_i64Tick[0][0]);
...
#if QLZ_COMPRESSION_LEVEL == 1
        {
            ...
            //gos_time_get_tick(&g_i64Tick[0]);
            hash = hash_func(fetch);
            //gos_time_get_tick(&g_i64Tick[1]);
            //g_i64Tick[2] += g_i64Tick[1] - g_i64Tick[0];
    ...
    //g_i64Tick[2] = gos_get_uptime_1ns();
    //g_i64Tick[2] += g_i64Tick[1] - g_i64Tick[0];

    for (i=1; i<=8; i++)
    {
        printf("\n time %d = %u, num = %u", i, gos_time_get_interval_ms(0, g_i64Tick[i][1]), (UINT32)g_i64Tick[i][2]);
    }

    //g_i64Tick[5][1] += g_i64Tick[5][0] - g_i64Tick[0][0];
