...
#define GET_PBYTE(x)        ((unsigned char *)x)
#define GET_BYTE(x,index)   *(((unsigned char *)x)+index)
...
#define TO_SHORT(ptr)   (*((short*)(ptr)))
#define TO_INT(ptr)     (*((int*)(ptr)))

#define same_6(s) (TO_INT(s) == TO_INT(s+1) && TO_INT(s) == TO_INT(s+3))
#endif
...
#if QLZ_COMPRESSION_LEVEL == 2
#define hash_func(i)    (((i >> 9) ^ (i >> 13) ^ i) & (QLZ_HASH_VALUES - 1))
#else
#define hash_func(i)    (((i >> 12) ^ i) & (QLZ_HASH_VALUES - 1))
#endif

#ifndef X86X64

#define fast_read_1(src, bytes)     GET_PBYTE(src)[0]
#define fast_read_2(src, bytes)     (GET_PBYTE(src)[0] | (GET_PBYTE(src)[1] << 8) )
#define fast_read_3(src, bytes)     (GET_PBYTE(src)[0] | (GET_PBYTE(src)[1] << 8) | (GET_PBYTE(src)[2] << 16))
#define fast_read_4(src, bytes)     (GET_PBYTE(src)[0] | (GET_PBYTE(src)[1] << 8) | (GET_PBYTE(src)[2] << 16) | (GET_PBYTE(src)[3] << 24))

static __inline ui32 fast_read1(void const *src, ui32 bytes)
{
...
#else
#define fast_read_1(src, bytes)     (*((ui32*)(src)))
#define fast_read_2(src, bytes)     (*((ui32*)(src)))
#define fast_read_3(src, bytes)     (*((ui32*)(src)))
#define fast_read_4(src, bytes)     (*((ui32*)(src)))

#define fast_read1(src, bytes)  (((bytes) > 0 && (bytes) <= 4)?*((ui32*)(src)):0)
#endif

static __inline ui32 hashat(const unsigned char *src)
{
    ui32 fetch, hash;
    fetch = fast_read_3(src, 3);
...
/*
#define fast_write_4(f, dst)     fast_write(f, dst, 4)
#define fast_write_3(f, dst)     fast_write(f, dst, 3)
#define fast_write_2(f, dst)     fast_write(f, dst, 2)
#define fast_write_1(f, dst)     fast_write(f, dst, 1)

*/

#define fast_write_4(f, dst)    \
do\
{\
    GET_BYTE(dst, 0) = (unsigned char)(f);\
    GET_BYTE(dst, 1) = (unsigned char)((f) >> 8);\
    GET_BYTE(dst, 2) = (unsigned char)((f) >> 16);\
    GET_BYTE(dst, 3) = (unsigned char)((f) >> 24);\
}while(0);

#define fast_write_3(f, dst)   \
do\
{\
    GET_BYTE(dst, 0) = (unsigned char)(f);\
    GET_BYTE(dst, 1) = (unsigned char)((f) >> 8);\
...
#define fast_write_1(f, dst)    *((unsigned char*)dst) = (unsigned char)(f)

#else

#define fast_write_4(f, dst)     *((ui32*)dst) = (f)
#define fast_write_3(f, dst)     *((ui32*)dst) = (f)
#define fast_write_2(f, dst)     *((ui16*)dst) = (ui16)(f)
#define fast_write_1(f, dst)     *((unsigned char*)dst) = (unsigned char)(f)
...
size_t qlz_size_decompressed(const char *source)
{
    ui32 n, r;
    n = (((*source) & 2) == 2) ? 4 : 1;
    //fast_read(source + 1 + n, n);
    if (n == 4)
    {
        r = fast_read_4(source + 1 + n, n);
    }
    else
    {
        r = fast_read_1(source + 1 + n, n);
    }
...
size_t qlz_size_compressed(const char *source)
{
    ...
    if (n == 4)
    {
        r = fast_read_4(source + 1, n);
    }
    else
    {
        r = fast_read_1(source + 1, n);
    }
...
static size_t qlz_compress_core(const unsigned char *source, unsigned char *destination, size_t size, qlz_state_compress *state)
{
    ...
    ui32 hash, cached;
    size_t remaining;
    qlz_hash_compress *phash;
    const unsigned char *old_src;
    size_t matchlen;
    ...
    while(src <= last_matchstart)
    {
        if ((cword_val & 1) == 1)
        {
            ...
            fast_write_4((cword_val >> 1) | (1U << 31), cword_ptr);
...
#if QLZ_COMPRESSION_LEVEL == 1
        hash = hash_func(fetch);
        phash = &state->hash[hash];
        cached = fetch ^ phash->cache;
        phash->cache = fetch;

        o = phash->offset + OFFSET_BASE;
        phash->offset = CAST(src - OFFSET_BASE);

#ifdef X86X64
        if ((cached & 0xffffff) == 0 && o != OFFSET_BASE && (src - o > MINOFFSET || (src == o + 1 && lits >= 3 && src > source + 3 &&
            //same(src - 3, 6))))
            same_6(src - 3))))
        {
            if(cached != 0)
            {
#else
        if (cached == 0 && o != OFFSET_BASE && (src - o > MINOFFSET || (src == o + 1 && lits >= 3 && src > source + 3 &&
           // same(src - 3, 6))))
            same_6(src - 3))))
        {
            if (*(o + 3) != *(src + 3))
            ...
            else
            {
                ...
                if (*(o + matchlen) == *src)
                {
                    ...
                    if(*(o + matchlen) == *src )
                    {
                        remaining = last_byte - UNCOMPRESSED_END - (src - 5) + 1;
                        if (remaining > 255)
                        {
                            remaining = 255;
                        }
                        ...
                        while(*(o+matchlen) == (*src) && (matchlen < remaining))
                        {
                ...
                if (matchlen < 18)
                {
                    ...
                    fast_write_2((ui32)(matchlen - 2) | hash, dst);
                    dst += 2;
                }
                else
                {
                    //fast_write((ui32)(matchlen << 16) | hash, dst, 3);
                    fast_write_3((ui32)(matchlen << 16) | hash, dst);
                    dst += 3;
                }
            }
            fetch = fast_read_3(src, 3);
...
#elif QLZ_COMPRESSION_LEVEL >= 2
        {
            ...
            if (offset2 < src - MINOFFSET && c > 0 && ((fast_read_3(offset2, 3) ^ fetch) & 0xffffff) == 0)
            {
                matchlen = 3;
            ...
            for (k = 1; k < QLZ_POINTERS && c > k; k++)
            {
                ...
                if (((fast_read_3(o, 3) ^ fetch) & 0xffffff) == 0 && o < src - MINOFFSET)
#elif QLZ_COMPRESSION_LEVEL == 2
                if (*(src + matchlen) == *(o + matchlen) && ((fast_read_3(o, 3) ^ fetch) & 0xffffff) == 0 && o < src - MINOFFSET)
#endif
                {
                    m = 3;
    ...
    for (;;)
    {
        ...
        if (cword_val == 1)
        {
            ...
            if (src + CWORD_LEN - 1 > last_source_byte)
                return 0;
#endif
            cword_val = fast_read_4(src, CWORD_LEN);
            src += CWORD_LEN;
        }
...
void qlz_print_marco()
{
    printf("QLZ_COMPRESSION_LEVEL = %u\n", QLZ_COMPRESSION_LEVEL );
    printf("QLZ_STREAMING_BUFFER  = %u\n", QLZ_STREAMING_BUFFER  );
    printf("QLZ_HASH_VALUES       = %u\n", QLZ_HASH_VALUES       );
    printf("QLZ_POINTERS          = %u\n", QLZ_POINTERS          );

#ifdef QLZ_PTR_64
    printf("QLZ_PTR_64            = yes\n");
#else
    printf("QLZ_PTR_64            = no\n");
#endif

#ifdef QLZ_MEMORY_SAFE
    printf("QLZ_MEMORY_SAFE       = yes\n");
#else
    printf("QLZ_MEMORY_SAFE       = no\n");
#endif
