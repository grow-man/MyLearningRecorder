...
/// https://github.com/RT-Thread-packages/quicklz/blob/master/
...
#define COMPRESS_BUFFER_SIZE           (1024 *1024 *8)
#define DCOMPRESS_BUFFER_SIZE          (1024 *1024 *8)
...
static int quicklz_compress_file(int fd_in, int fd_out)
{
    ...
    unsigned char *cmprs_buffer = NULL, *buffer = NULL;
    unsigned char buffer_hdr[BLOCK_HEADER_SIZE] = { 0 };
    ...
    cmprs_buffer = (unsigned char *) malloc(COMPRESS_BUFFER_SIZE + BUFFER_PADDING);
    buffer = (unsigned char *) malloc(COMPRESS_BUFFER_SIZE);
...
static int quicklz_decompress_file(int fd_in, int fd_out)
{
    ...
    unsigned char *dcmprs_buffer = NULL, *buffer = NULL;
    unsigned char buffer_hdr[BLOCK_HEADER_SIZE] = { 0 };
    ...
    {
        printf("[qlz] decomprssion file size : %d error!\n", file_size);
        ret = -1;
        goto _dcmprs_exit;
    }

    dcmprs_buffer = (unsigned char *) malloc(DCOMPRESS_BUFFER_SIZE);
    buffer = (unsigned char *) malloc(DCOMPRESS_BUFFER_SIZE + BUFFER_PADDING);
...
int quicklz_test(int argc, char ** argv)
{
    ...
    if (fd_in < 0)
    {
        printf("[qlz] open the input file : %s error! and errinfo is %s\n", argv[2], strerror(errno));
        ret = -1;
        goto _exit;
    }
